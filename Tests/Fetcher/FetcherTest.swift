...
class FetcherTest: QuickSpec {
    ...
    private struct GitHubEndpoints: EndpointProvider {
        ...
        static let nonGithubEndpoint = GET<Void, String>("ftp://test", modifiers: BaseUrl.Ignore)
    ...
    override func spec() {
        describe("Fetcher") {
            ...
            beforeEach {
                ...
                fetcher = Fetcher(requestPerformer: performer, callbackQueue: DispatchQueue.global(qos: .background))
                fetcher.register(requestModifiers: BaseUrl(baseUrl: "https://api.github.com"))
            }
            it("supports Void to String request") {
                performer.register(endpoint: GitHubMockEndpoints.zen)
                var zensponse: String?
                fetcher.request(GitHubEndpoints.zen) { response in
            ...
            it("supports Void to Object request") {
                ...
                fetcher.request(GitHubEndpoints.userProfile(name: "brightify")) { response in
            ...
            it("supports absolute url in endpoint") {
                ...
                fetcher.request(GitHubEndpoints.nonGithubEndpoint) { response in
            ...
            it("supports custom RequestEnhancers") {
                ...
                fetcher.register(requestEnhancers: firstEnhancer)
                fetcher.register(requestEnhancers: secondEnhancer)
                
                fetcher.request(GitHubEndpoints.test) { _ in }
